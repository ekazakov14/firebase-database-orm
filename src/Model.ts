...
abstract class Model<T> {
  ...
  public static getFields(filter: Partial<FieldDescriptor> = {}): FieldDescriptor[] {
    const fields = Reflect.getMetadata(FIELDS_KEY, this) as FieldDescriptor[];

    return Object.keys(filter).reduce((resultFields: FieldDescriptor[], key) => (
      resultFields.filter((field) => field[key] === filter[key])
    ), fields);
  }
